...
function ImagePreview({product}) {
  ...
  return (
    <div>
        <div className='relative h-[500px] sm:h-[600px] xl:h-[calc(100vh-200px)] w-[100%] rounded-md overflow-hidden'>
        ...
        <div className='flex gap-2 items-center h-[120px]'>
            <div className='h-[100px] relative w-[100px] rounded-md overflow-hidden'>
                {product.image ? <Image src={product.image} objectFit="cover" layout='fill' alt='img'/>: null}
            </div>
            <div className='grow flex items-center gap-2 overflow-x-auto'>
                {
                    product.moreImage.map((prod, i) => (
                        <div key={i} className='h-[100px] relative w-[100px] rounded-md overflow-hidden'>
